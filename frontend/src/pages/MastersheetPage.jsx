...
function MastersheetTable({ data, rows, stats, title }) {
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      {title && <div className="ms-section-title">{title}</div>}
      <div className="ms-stats">
        <div className="stat-box"><span className="stat-num">{stats?.sat_count ?? stats?.total_students}</span><span>Effectif</span></div>
        <div className="stat-box pass"><span className="stat-num">{stats?.pass_count}</span><span>Admis</span></div>
        <div className="stat-box fail"><span className="stat-num">{stats?.fail_count}</span><span>Refusés</span></div>
        <div className="stat-box avg"><span className="stat-num">{stats?.class_average ?? '—'}</span><span>Moy. Classe</span></div>
        <div className="stat-box rate"><span className="stat-num">{stats?.pass_rate}%</span><span>Taux Réussite</span></div>
        <div className="stat-box"><span className="stat-num">{stats?.highest_avg ?? '—'}</span><span>Moy. Max</span></div>
        <div className="stat-box"><span className="stat-num">{stats?.lowest_avg ?? '—'}</span><span>Moy. Min</span></div>
      </div>
        ...
        <table className="ms-table">
          <thead>
            <tr>
              ...
              {data.subjects?.map(s => (
                <th key={s.id} className="ms-subject-header">
                  {s.code || s.name}{s.name_fr ? ` / ${s.name_fr}` : ''}
                </th>
              ))}
            ...
            <tr className="ms-coef-row">
              <th></th><th>Coefficients</th>
              {data.subjects?.map(s => <th key={s.id}>{s.coefficient}</th>)}
              <th>{data.total_coefficients}</th><th>/20</th><th></th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, idx) => (
              <tr key={row.student_id} className={idx % 2 === 0 ? 'ms-row-even' : 'ms-row-odd'}>
                ...
                {row.cells?.map((cell, ci) => (
                  <td key={ci} className={cell.points === null ? 'cell-null' : cell.avg_score >= 10 ? 'cell-pass' : 'cell-fail'}>
                    {cell.points !== null ? cell.points.toFixed(2) : '—'}
                  </td>
                ))}
                <td className="ms-tm">{row.tm?.toFixed(2) ?? '—'}</td>
                <td className={`ms-avg ${row.avg >= 10 ? 'avg-pass' : 'avg-fail'}`}>{row.avg?.toFixed(2) ?? '—'}</td>
          ...
          <tfoot>
            <tr className="ms-footer-row">
              ...
              <td>—</td><td>{stats?.class_average?.toFixed(2) ?? '—'}</td><td>—</td>
            </tr>
          </tfoot>
        </table>
      </div>
      <div className="ms-analysis">
        <table className="ms-analysis-table">
          <tbody>
            <tr>
              <th>Effectif total</th><td>{stats?.total_students}</td>
              <th>Admis (≥10/20)</th><td className="pass-cell">{stats?.pass_count}</td>
              <th>Refusés (&lt;10/20)</th><td className="fail-cell">{stats?.fail_count}</td>
            </tr>
            <tr>
              <th>Moy. de classe</th><td>{stats?.class_average?.toFixed(2) ?? '—'}</td>
              <th>Moy. la plus haute</th><td>{stats?.highest_avg?.toFixed(2) ?? '—'}</td>
              <th>Moy. la plus basse</th><td>{stats?.lowest_avg?.toFixed(2) ?? '—'}</td>
            </tr>
            <tr>
              <th>Taux de réussite</th><td><strong>{stats?.pass_rate}%</strong></td>
              <th>Distinction (≥16)</th><td>{rows.filter(r => r.avg >= 16).length}</td>
              <th>Insuffisant (&lt;8)</th><td>{rows.filter(r => r.avg !== null && r.avg < 8).length}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default function MastersheetPage() {
  ...
  useEffect(() => {
    get('/classes').then(r => {
      ...
      allClasses.forEach(c => {
        ...
        if (id && !seen.has(id)) { seen.add(id); uniqueYears.push({ id, label: c.academic_year }); }
      });
  ...
  const handleYearChange = (e) => { setYearId(e.target.value); setClassId(''); setData(null); setError(''); };

  const generate = async () => {
    if (!classId || !yearId) { setError('Select year and class first'); return; }
    setLoading(true); setError(''); setData(null);
    const res = await get(`/mastersheet?class_id=${classId}&academic_year_id=${yearId}&view_mode=${viewMode}`);
    setLoading(false);
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      if (scaledH <= pdfH) {
        const yOff = (pdfH - scaledH) / 2 + margin;
        pdf.addImage(imgData, 'PNG', margin, yOff, pdfW, scaledH);
      } else {
        let yPos = 0; let page = 0;
        const pageSliceH = pdfH / ratio;
        while (yPos < imgH) {
          if (page > 0) pdf.addPage();
          const sliceH = Math.min(pageSliceH, imgH - yPos);
          const tmp = document.createElement('canvas');
          tmp.width = imgW; tmp.height = Math.ceil(sliceH);
          tmp.getContext('2d').drawImage(canvas, 0, yPos, imgW, sliceH, 0, 0, imgW, sliceH);
  ...
  return (
    <div className="tab-content">
      ...
      <div className="filter-bar no-print">
        ...
        <div className="form-group">
          <label>Class</label>
          <select value={classId} onChange={e => { setClassId(e.target.value); setData(null); }}
        ...
        <div className="form-group">
          <label>View</label>
          <select value={viewMode} onChange={e => { setViewMode(e.target.value); setData(null); }} className="select-input">
        ...
        <div className="mastersheet-wrapper" id="ms-printable">
          <div className="ms-header">
            ...
            <div className="ms-title-block">
              ...
              <p style={{ marginTop: 6, fontSize: 12, fontWeight: 'bold', color: '#1E88C7' }}>{data.view_label}</p>
            </div>
          ...
          <MastersheetTable
            data={data} rows={data.rows} stats={data.stats}
          ...
          {isT3 && data.annual_rows && (
            <>
              ...
              <MastersheetTable
                data={data} rows={data.annual_rows} stats={data.annual_stats}
          ...
          <div className="ms-signature-block">
            <div className="sig-box">
              ...
              <div className="sig-line"></div><p>Name &amp; Stamp</p>
            </div>
            <div className="sig-box">
              <p><strong>Responsable des Notes / Results Officer</strong></p>
              <div className="sig-line"></div><p>Name &amp; Signature</p>
            </div>
            <div className="sig-box">
